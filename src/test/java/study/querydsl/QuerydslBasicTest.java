...
public class QuerydslBasicTest {
    ...
    @Autowired
    EntityManager em;
    JPAQueryFactory jpaQueryFactory; // 필드로 사용하는 방법 권장 (동시성 문제 없음)
    ...
    public void startQuerydsl() {
        ...
        QMember m1 = new QMember("m1"); // 같은 테이블을 조인해야하는 경우에만 선언해서 사용하고 나머지는 static import를 사용 권장

        Member findMember = jpaQueryFactory
                .select(member) // static import로 사용하는 것을 권장
                .from(member)
    ...
    @Test
    public void search() {
        Member fidnMember = jpaQueryFactory
                .selectFrom(member)
                .where(member.username.eq("member1")
...
//                .where(team.name.eq("teamA"))
    ...
    public void fetchJoinUser() {
        ...
        boolean loaded = emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam());
        assertThat(loaded).as("페치 미적용").isTrue();
    }
